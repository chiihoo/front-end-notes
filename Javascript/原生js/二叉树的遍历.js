...
var preorderTraversal = function(root) {
  ...
  stack.push(root);
  while(stack.length){  //不能加上root!=null，因为stack空了的话，如果还要进行循环，那么stack.pop()会报错
    root=stack.pop();
    res.push(root.val);
    if(root.right){     //后进先出
...
var inorderTraversal = function(root) {
  let res=[];
  let stack=[];
  while(root!=null || stack.length!=0){   //在刚好运行到根节点的右边时，stack.length==0，但root!=null
    while(root!=null){
      stack.push(root);
      root=root.left;
    }
...
var postorderTraversal = function(root) {
  ...
  while(root!=null || stack.length!=0){
    ...
    if(root.right==null || root.right==lastVisit){  //如果没有右子树或者右子树已经访问过了，就把root添加进res
