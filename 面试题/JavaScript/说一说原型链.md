每个对象都有一个叫`__proto__`的内置属性，并且这个`__proto__`指向创建它的构造函数的原型对象`prototype`，这样就构成了一个链式的结果，原型链的最顶端是`null`。当读取一个对象的属性的时候，会先从对象实例上找，没有就沿着原型链往上找，直到到达顶端null为止。原型链可以用来实现继承
