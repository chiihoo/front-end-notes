...
// 2.快速排序：需要递归，时间复杂度O(nlog(n))，最坏O(n^2) 空间复杂度为递归的深度O(log(n))，最坏情况下为O(n)
...
// 如果刚开始从左往右扫，找第一个大于基准数的值，到i与j相遇时，找到的值可能并没有大于基准值的情况
...
// 堆排序升序过程就是先建大根堆，可以从前往后遍历，只要该结点比父结点大，就交换位置
// 建好之后大根堆堆顶元素就是最大的，与最后一个数交换位置，现在最后一个数是最大的了
// 调整堆，从根结点往下比较子结点，如果子结点比根结点小，就换位置，再继续往下比较
// 但是调整的范围不要包含最后一个数，用heapSize参数来控制
// 这样比较到最后，整个数组就是升序排列的，并且没有用额外空间，空间复杂度为O(n)

// 我下面的写法建堆是从后往前建的，从最后一个结点开始往前
