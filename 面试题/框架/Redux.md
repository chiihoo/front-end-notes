...
```
设计理念
redux可以分为很多模块，每个模块有自己的reducer和state，reducer就是对state进行操作的一个处理中心。之后将每个reducer用combineReducers进行合并，再用createStore(reducers)创建一个全局的store。
...
之后组件可以通过connect来获取这个store对象，通过store.getState()和store.dispatch来获取state存储和dispatch函数，通过传入的mapStateToProps和mapDispatchToProps提取要用到的state与封装了dispatch的方法
```
