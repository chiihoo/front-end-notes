...
3. **DNS域名解析**
   ...
   4. 如果还是找不到，本地DNS服务器就会去根域名服务器进行查询。根域名服务器收到请求，会返回对应的顶级域名服务器的地址，之后本地DNS服务器像顶级域名服务器查询，顶级域名服务器收到请求，又会返回二级域服务器的地址，直到最精确的那台DNS，得到查询结果，最后返回给客户端。
...
* **为什么外链css要放在html文件的header标签里面，而script标签要放在body的最后面？**
  ...
  **css加载**，不会阻塞dom树的解析（异步加载时dom照常构建），但是**会阻塞render树的渲染**，render树渲染时需要等css加载完毕，因为render树需要css信息。

  

**为什么script标签要放在body里面而不是header里面？**

因为浏览器遇到body标签才会开始显示内容，如果script标签放在header里面，就必须等js代码完全加载完毕，才开始显示页面。可以用defer解决
